...
| 熟练的 AI 编程工程师 | 已掌握底层逻辑，快速回顾参考，Check List | 第一部分重点，第二部分在按需辅助自己工作、或用作培训案例 |
...
> **AI 是放大器，不是发动机。你有方向它放大十倍，你没方向它放大的是零。**
...
**关键判断**：Claude Code 就是你的工程团队，而你必须成为那个架构师——决定做什么、不做什么、用什么方式做、达到什么标准。

#### (3) 判断永远在人，AI 只负责执行

哪些模块必须有、哪些功能可以砍、数据模型怎么设计——这些判断来自自身经验积累。AI 可以帮我们把判断高效地变成代码，但**判断本身是我们自己的**。
...
**使用要诀**：每次给 AI 下任务前，先引用 CLAUDE.md 中的相关章节，不要让它"凭印象"发挥。
...
**为什么"不做什么"和"做什么"同等重要**：AI 默认会按"通用最佳实践"补全你没说的部分，往往导致过度设计。明确"不做什么"，等于给 AI 划了红线。
...
**三步缺一不可**：只查意图会放过 bug；只查质量会漏掉架构越界；只查边界会忽略功能缺失。
...
**核心心法**：拆得好，AI 一次做对；拆得差，反复返工。瓶颈不在 AI 能力，在你的思考质量。
...
> 工程可以复制，模型能力不能。**真正的壁垒不在工具怎么实现，而在模型能力，以及使用工具的人怎么思考。**

51 万行源码摊开在所有人面前，Claude Code 的竞争力一点没少。恰恰相反：**工具迭代得越快，掌握底层方法的人越值钱**——因为只有他们能在工具更替中始终保持生产力。
...
**关键认知**：它不是不能做好，是我没告诉它"好"的标准是什么。AI 默认按"通用最佳实践"补全你没说的部分——而对一个一周单人项目，"通用最佳实践"恰恰是过度设计。
...
它的输出质量发生了质的飞跃，**不是因为它变聪明了，而是因为我给它的输入变清晰了**。
...
**核心认知**：用 Claude Code 做项目，瓶颈从来不在 AI 的能力上，而在你的**思考质量**上。你想得越清楚，它做得越准确；你越模糊，它越跑偏。
...
**核心认知**：跨模块问题的根因往往不在"当前模块"，而在"模块之间的契约"。把契约（规范）一起喂给 AI，它才能看到全局。
...
<!--
图片：imgs/00_开篇_01：一人写一个简版Dify需要多久/ec1b48a79ea18a7211e1b5a38f987863_MD5.webp
...
**最后一句送给每一位读者**：你能收获的，不仅是怎么用好 Claude Code，更重要的是，**怎么做好那个下判断的人**。有了这个能力，就不会绑定任何一个工具。
