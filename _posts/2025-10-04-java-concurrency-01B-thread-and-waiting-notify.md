...
FutureTask的设计：由于Future只是接口，无法直接创建对象使用，因此引入了FutureTask。FutureTask实现了RunnableFuture接口，RunnableFuture继承了Runnable接口和Future接口，因此FutureTask：

- 可以作为Runnable被线程执行，将Callable包装成Runnable用于创建Thread
- 可以作为Future获取Callable的返回值（如代码倒数第二行的task.get()）
