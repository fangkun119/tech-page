...
AI 编程这两年冒出来不少方法论和工具：SDD、Harness、Vibe Coding、Context Engineering、ReAct、Test-Driven AI、Plan-then-Execute、AI-First Architecture。我的判断很直接：**当下真正成熟可教的方法论，只有两个，SDD 和 Harness Engineering**。这篇文章就把这两根支柱讲透。

这两个方法论不是同一回事，各自解决 AI 编程里一个不同的核心问题：

1. **SDD 解决"想"的问题**，把"想清楚自己要什么"这件事工程化，通过结构化的 spec 让 AI 拿到清晰的执行依据。
2. **Harness 解决"做"的问题**，把"AI 怎么做"这件事工程化，通过一套配置和约束让 AI 在合理边界内工作。
...
传统开发里，你想一点做一点，需求和实现紧密耦合在你的脑子里。AI 编程把这个耦合撕开了：**"想"全部留给你，"做"被外包给 AI**。
...
通过结构化的 spec（规格），把模糊需求变成 AI 能消化的清晰输入。spec 不是简单的需求文档，**它更像你熟悉的需求规格说明书加上接口契约**：有结构、有验收标准、有边界约束。AI 读完它，知道要做什么、做到什么程度、什么不该做、做完怎么验证。
...
Harness 的作用是给 AI 装上挽具，让它在合理边界内工作。**你可以把 Harness 理解成 CI/CD 流水线 + code review 制度 + 权限管理的合体**——这些都是你传统开发里已经在用的工程化机制，只是 AI 编程需要一套新的实现。
...
一句话收束：**SDD 告诉 AI 做什么，Harness 保证 AI 做对**。前者从需求侧约束，后者从执行侧约束。
...
1. **成熟**：有完整概念体系和经过验证的实践经验。
2. **可教**：有具体步骤、可用工具、可练动作。
3. **覆盖完整工程**：能贯穿从需求到上线的整条链路。
...
我的立场很明确：**学方法论不在多，在精**。先把这两个支柱在真实项目里跑熟，你有能力判断其他方法论的位置；而不是在十种半生不熟的方法论里来回跳。
...
讲到这里值得拎出一个判断：**SDD 的核心是思考方法，不是工具本身**。Spec-Kit 和 OpenSpec 给了 SDD 落地的工具，但不是所有场景都需要这些工具。当你对项目足够熟悉、规模可控、任务粒度天然小，SDD 的"想清楚再做"这个核心动作，在你脑子里就能完成，产出物不需要外化。这不是 SDD 失效，是 SDD 已经内化成你的工作习惯，不需要外部脚手架了。
...
**全局视角在谁手里**：自动化模式里，AI 试图同时拥有全局视角（拆任务）和局部视角（执行任务）。但 AI 的全局视角是从 spec 推断出来的，失真严重——你 spec 里没说的东西、你脑子里默认的约束、你团队的真实优先级，AI 都不知道。AI 用一个不完整的全局视角拆出来的任务，经常错过关键路径。手动控制模式和轻量手工模式里，全局视角永远在你手里，AI 只在局部工作。这种分工让 AI 待在它擅长的位置（执行），让你待在你擅长的位置（判断）。

**AI 误差的放大倍数**：自动化模式有两轮推断叠加，误差是相乘而不是相加。拆任务对了 80%，执行任务对了 80%，最终产出只有 64% 接近你真正想要的。这是自动化模式最难解决的根本问题。手动控制模式和轻量手工模式都只有一轮推断——拆任务由你做（没有 AI 误差），AI 只在执行时推断一次，最终产出质量上限就是 AI 单次执行的质量，不会被前置环节稀释。

**三种模式服务不同水平的工程师，这件事不能藏着说**：新手用自动化模式更顺，因为 AI 帮你补了你还没建立起来的判断；中级工程师用手动控制模式更稳，工具能承载你不能全部装在脑子里的内容；资深工程师在熟悉的项目上用轻量手工模式效率最高，因为你的判断 AI 替代不了，工具反而会拖你后腿。
...
最后一条判断：**三种模式都该会，场景不同切换用**。它们不是替代关系，是互补关系。Spec-Kit 给你完整流程的脚手架，适合不熟的场景；OpenSpec 给你 delta spec 和手动控制的灵活性，适合改造和系统级；轻量手工模式给你最高的产出可控性，适合熟悉的场景。一个成熟的 AI 编程工程师工具箱里，三种模式都该有，根据场景选最合适的。
...
**什么时候用**：任何严肃的项目都该配。Hooks 是把"工程化"从概念变成具体动作的关键机制。**没有 Hooks 的 AI 编程，本质上还是手工作坊；有了 Hooks，AI 编程才进入工业化阶段**。
...
但要补充一句：**不是所有场景都需要六个工具全上**。Harness 的工具是按场景取用的，不是堆得越多越好：
...
我的判断：**工具该用才用，不用就不用，这种"克制"才是真正的工程师姿态**。工具堆得多不等于工程化做得好，工程化做得好是知道什么时候用什么工具。
...
两个方法论共享一个底层假设：**AI 不是被信任的，是被工程化使用的**。

不是"AI 像同事一样自己会做事"，是"**AI 是一个能力极强但缺判断边界的执行者，需要被精确指挥、被严格约束、被持续校验**"。
...
**SDD + Harness 是 AI 编程里 ROI 最高的两件事**。你找不到比这投入产出比更高的工程化动作了。

### 6.4 收束：把这两根支柱练到肌肉记忆

回到最开头那个判断。AI 编程方法论很多，**但当下真正成熟可教的就两个：SDD 和 Harness。一个解决"想"的问题，一个解决"做"的问题，合起来是完整闭环**。
