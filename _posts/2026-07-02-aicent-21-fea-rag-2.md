...
- 三步的顺序不能反。如果先测新功能再测旧功能，旧功能出了问题，无法判断是这次改动导致的，还是原本就有的。
...
**操作要领**：多步骤串联任务，把每个环节拆成独立方法（或独立步骤），管线方法只负责串联和状态管理。
...
**操作要领**：检索增强（RAG）注入 system prompt 时，要加一句明确的边界约束——"如果资料中没有相关信息，直接说没有找到，不要编造"。

**思考关注点**：

- 没有这个约束，LLM 在检索结果不充分时会用训练知识补充，而且不会告诉用户它在补。
- 加了这句约束，LLM 知道自己的边界。RAG 不只让 LLM 能引用文档，也让它知道自己不知道什么。
...
最终圈定的改动范围：**只改 ChatService 的 buildMessages 方法，其他八步不动。** 流式调用、SseEmitter 转发、消息存储、Redis 上下文管理，全部不碰。把这个约束写进给 Claude Code 的指令，是防止改动扩散的第一道防线。
...
- **knowledge_base 和 document** 是业务数据，有完整的增删改查，走 MyBatis-Plus，放 MySQL。
- **document_chunk** 是向量数据，只有批量写入和相似度查询两种操作，需要 pgvector 的向量检索能力，放 PostgreSQL。用 JdbcTemplate 直接写 SQL 比配 MyBatis Mapper 更简单。
...
每个环节独立，管线只负责串联。这不只是为了代码整洁，更重要的是：某个环节出了问题时，能精确定位是哪一步，单独测试单独修复，而不是在一个几百行的大方法里翻来翻去。这个原则适用于任何多步骤数据处理管线（方法论 2.5）。
...
这种情况下，遵循增量开发三步走：**圈定改动范围 → 验证旧功能没坏 → 验证新功能生效**。三步顺序不能乱，每一步都是对前一步的护栏。
