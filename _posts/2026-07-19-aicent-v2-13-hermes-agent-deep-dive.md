...
Hermes Agent 是 Nous Research 开发的开源、自我改进的个人 AI Agent，它跟人一起成长。
...
Hermes 的定位很清晰：一个属于你个人的、跟你长期一起成长的 AI Agent。
...
这是教科书级的抽象隔离，类比 SLF4J（日志门面）或 JDBC（数据库驱动）：**底层差异藏到接口后面，上层代码不用改**。换模型像换数据库驱动一样，业务逻辑一行不动。
...
这是个性化的核心。**每个人的 Hermes 实例 system prompt 都不一样**，因为每个人的 MEMORY/USER/SKILLS 都不一样。两个用户用同一份代码、同一个模型，跑出来的 Agent 行为可以完全不同。
...
类比传统 Runbook：**Runbook 是团队手写的、人维护的**，跑偏了要靠新人反馈。Hermes 的 skill 是 Agent 自己写、自己改、自己归档的——**整个生命周期不需要人维护**。这是质变。
...
**普通 LLM-wrapper 的能力是「LLM 给的」**。你封装一层 prompt、套一层工具，模型多强它就多强。**天花板就是模型本身**——你把 GPT-4 换成 GPT-5，它变强；不换，它永远停在原地。再怎么用、用多久，都不会长出新能力。

**Hermes 的能力是「LLM + 自己积累的 skill 库」**。模型本身只是地基，skill 库是盖在上面的楼。**地基不变，楼可以一直往上盖**——用得越久，skill 越多、越精、越贴合你的工作流。**天花板随使用时间上升**。

这才是 closed learning loop 的本质：**它把模型能力的静态边界，变成了随时间增长的能力飞轮**。
...
类比 CI/CD：**工程师改的是代码，不是编译器**。编译器（模型）不动，但代码（skill）每跑一轮都在变好。CI/CD 让代码质量随时间上升，closed learning loop 让 Agent 能力随时间上升——**机制完全同构**。

#### (5) 这为什么是分水岭

我的看法是，**closed learning loop 是 Hermes 跟所有 LLM-wrapper 的分水岭**。前者越用越强，后者再怎么用都还是模型本身的能力。

把时间维度拉长看：一个用了半年的 Hermes，跟一个刚部署的 Hermes，**不是同一个 Agent**。前者带着你半年的工作流记忆、几百个针对性 skill、踩过并被 patch 过的坑库。后者是空白。

这是普通 chatbot 永远做不到的。它们每次对话都从零开始，再聊一万次也是 GPT/Claude 本身。**Hermes 的护城河不是模型，是时间**——你用得越久，它跟你的耦合越深，迁移成本越高，价值也越大。
...
这些设计反映一个事实：**Hermes 是真实生产用的 Agent，不是 demo 项目**。每一项都对应一个真实攻击面——容器逃逸、prompt injection、供应链投毒、webhook 伪造。**demo 不需要管这些，生产必须管**。
...
这不是「两个业务」，是「一个闭环」。市面上绝大多数 Agent 厂商只造整车（用别家的模型），市面上绝大多数模型厂商只造发动机（不知道下游怎么用）。Nous Research 两头都做，数据能在自己手里转起来。
...
我的判断是，这种「Agent 反哺模型」的闭环是 Nous Research 做 Hermes 的真正野心——它不只是工具，是数据飞轮。一个只做 Agent 的公司，天花板是上游模型；一个只做模型的公司，天花板是不懂下游场景。两边都做、数据能自循环的公司，天花板是飞轮转速本身。
