...
三条路径、三种回答风格，都由同一个工作流配置驱动。查 `workflow_node_run` 表，每个节点的输入输出、执行耗时、成功失败状态都清清楚楚：`classify` 节点 1200ms，`router` 节点 2ms，`aftersale` 节点 3400ms。出了问题能精确定位到哪个节点，这种可观测性是单 Prompt Agent 做不到的。
...
不要从 DAG、图论这些学术概念入手，会让人绕进去。需要的是能直接对应到代码的解释。
...
不是所有方案都适合你的场景。调研的价值在于知道有哪些选项、各自的代价是什么，然后做出有理由的选择。例如 n8n 的 Redis Queue 是为多 Worker 水平扩展设计的；Temporal 的事件溯源是为小时/天级工作流设计的；Dify 的代码沙箱是为用户自定义代码节点设计的——这些 Hify 都不需要。
...
盲目追求一次写完，出了问题不知道是哪一步错的。分步实现让每一步的错误都能被快速定位和修复。
...
正常路径交给 Claude Code 写，基本能写对，不用花太多精力。review 的精力集中放在「如果这一步的输入不是预期的会怎样」上，逐个追问边界情况。例如工作流执行引擎要检查四类异常分支：
...
| LLM 节点调用失败 | `workflow_node_run` 记录 FAILED，`workflow_run` 也标记 FAILED |

#### (2) 为什么

Claude Code 写正常路径没问题，但边界情况大概率遗漏。这些问题只有在 review 时逐个追问才能补全。
...
问「退货政策是什么」单 Prompt 够用；问「我的订单到哪了」必须走工作流——因为答案在数据库里，不在 LLM 脑子里。
...
| 决策 | 选择 | 原因 |
|------|------|------|
| 连线如何引用节点 | 用 `node_key` 字符串而非数据库 id | 前端拖拽时不依赖自增顺序，避免 id 还没生成就要引用的问题 |
| 节点配置存储方式 | 用 JSON 字段而非拆列 | 不同类型节点配置格式差异大，强行拆列会让表结构难以维护，每加一种节点类型就要改表结构 |

##### ④ 类型安全解析方案

采用 `sealed interface + record` 做类型安全解析。`switch` 模式匹配漏写任何一个 case 编译就报错，从编译期保证节点类型解析的完整性。这个原则与第 12 篇 `auth_config` 的处理方式一致。
...
Hify 选轻量同步引擎，参考 Dify 的 `VariablePool` 模式，不引入消息队列。原因：Hify 的节点类型是固定的几种，不需要代码沙箱；工作流是一次对话响应的一部分，不需要长周期编排；不做多 Worker 水平扩展，不需要消息队列。
...
| 异常场景 | 期望保护 | review 重点 |
|----------|----------|-------------|
| a. 条件分支所有条件都不匹配 | 有 `defaultTarget` 或合理报错 | Claude Code 是否给出了处理逻辑 |
| b. `targetNodeKey` 指向不存在的节点 | 不空指针，`nodeMap.get` 返回 null 时抛异常 | 保护逻辑是否到位 |
| c. 工作流里有环（A→B→A） | 50 步限制生效，不死循环 | 步数上限是否生效 |
| d. LLM 节点调用失败 | `workflow_node_run` 记 FAILED，`workflow_run` 也标记 FAILED | 失败状态是否正确传播 |
