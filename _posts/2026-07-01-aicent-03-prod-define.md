...
从这一篇开始，我们正式动手做项目。但"动手"不是"写代码"——动手做的第一件事，是想清楚要做什么。
...
所以这一篇要做的事情就是：**在动第一行代码之前，把产品边界、技术选型、运维预期全部想清楚，写进 CLAUDE.md**。产品定义这一阶段就可以用 AI，不是只有写代码时才用。
...
用 AI 编程时，这两种失败都会被放大：AI 不会替你拒绝需求，它会"贴心地"把你没说的功能也补上。
...
产品定义阶段不解决"模块怎么分"（那是架构设计阶段的事），也不解决"代码怎么写"（那是实现阶段的事）。它只解决一个问题：**我们要做的到底是什么东西**。
...
| 步骤 | 目标 | AI 角色 | 人角色 | 产出 |
|------|------|---------|--------|------|
| **1. 调研标杆** | 理解领域全貌，知道什么是核心、什么是边缘 | 快速梳理标杆产品的功能模块 | 拿着 AI 清单去比对官方文档/GitHub，修正过时描述、补上遗漏 | 标杆功能全景图 |
| **2. 功能取舍** | 从标杆功能中圈出自己的范围 | 基于你的约束，给出"必须做 / 降级简化 / 砍掉"三档初筛 | 逐项决策，否决不合理的建议 | 三档功能清单 + 一句话产品定位 |
| **3. 技术选型** | 选匹配约束的技术栈，不追新 | 多维度对比候选方案 | 基于"熟、够用、不分裂"三准则做最终选择 | 前后端 + 容器化技术栈定稿 |
| **4. 运维预期** | 提前想清楚怎么跑，反向影响架构 | 估算 QPS、列缓存策略、查漏运维事项 | 识别真正的瓶颈（往往不是并发），定下部署与监控方案 | 部署方式、QPS 预期、缓存策略、监控方案 |
| **5. 写进 CLAUDE.md** | 把前面四步的结论落成文字，作为后续 AI 协作的上下文根 | 生成初稿 | 检查修改后定稿，并在项目演进中持续更新 | CLAUDE.md 项目概述部分 |
...
这一问防止核心功能做满。核心功能也要分级—— MVP 版本、可用版本、完整版本。判别方法：**当前阶段用户能用就行，还是必须做得精美**。能用就行的，做最简版。
...
如果是了解 Dify 的人，就会知道这份清单比较完整。但如果我们不了解 Dify 呢？此时原则是：**AI 给的信息不能直接信，你要验证**。
...
你会发现 Claude Code 的输出不是简单的"做 / 不做"二分法，而是分成了三档：**必须做、降级简化、砍掉**。这个分档方式本身就有价值——我自己一开始想的是二分法。
...
它说得对。如果只能通用对话，团队直接用 ChatGPT 就行了。**私有知识的接入才是内部部署的核心价值**。采纳，但降级：一期只支持 TXT 文档，分块用最简单的固定长度。
...
Spring Boot 单实例轻松应对。**真正的瓶颈是 LLM 响应延迟（3-30 秒 / 次）占用连接——这不是 QPS 问题，是长连接管理问题**。
...
这里有一个环节很重要：**你得看得懂上面这些建议**。如果你看不懂，整个项目就会失控。比如因为短期 QPS 不高，我放弃了做缓存，因为缓存会增加开发的复杂度。
...
这一步对应第 3 章 Check List 第 15-16 项。合格的 CLAUDE.md 项目概述必须包含四块：**做什么 / 不做什么 / 技术栈 / 运维预期**。少任何一块，后续 AI 协作都会缺上下文。
...
这一篇我们做了一件事：**在动第一行代码之前，把产品的边界想清楚了**。

```mermaid
flowchart LR
    A[调研标杆<br/>AI 梳理,你验证] --> B[功能取舍<br/>三问裁剪,AI 初筛你拍板]
    B --> C[技术选型<br/>AI 对比,你基于约束选择]
    C --> D[运维预期<br/>AI 查漏,你识别瓶颈]
    D --> E[写进 CLAUDE.md<br/>AI 初稿,你定稿]
    E --> F[可执行边界<br/>进入架构设计]
```

你会发现，这其实是一个**问对问题、鉴别问题**的过程。最难的就是这一步——怎么问对问题，怎么鉴别问题。这需要积累。这一点，这个系列不能帮到你太多，只能尽量告诉你我是怎么想的。举一反三的过程，需要你不断地学习、积累和反思。
