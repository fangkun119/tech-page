...
<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/4fd43aa467a8e93117c5306390d3b007_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">

leader 在群里 @ 你："给 Prompt 加个版本对比功能"，然后就下线了。你对着这十几个字发呆：什么是版本对比？对比什么？要做到什么程度？给谁用？——这种"被一句话砸过来、心里没底"的状态，就是本篇要解决的问题。

老项目里这种场景几乎是常态：需求来源是一句话，能不能写代码、改完会不会踩坑，全看拆需求这一步拆得怎么样。同样是拆这一句话，不同工程师交出来的东西差距巨大：有人交出<span style="color: red; font-weight: bold;">接口契约清晰、边界场景齐全、老项目约束标到位</span>的版本；有人交出"加个版本对比 + 三行接口签名"就完了。

### 1.1 三种最常见的翻车

同样是拆需求，为什么有人交出来的是清晰契约，有人交出来的是三行接口签名？下面是最常见的三种翻车。

#### (1) 三种翻车对照

| 翻车类型                                                          | 典型表现                                                                                | 后果                |
| ------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ----------------- |
| 翻车一：写得太抽象                                                     | "加个版本对比功能"，<span style="color: red; font-weight: bold;">接口签名不清，错误码、返回结构全靠开发猜</span> | 开发反复找产品确认，每次半天    |
| 翻车二：<span style="color: red; font-weight: bold;">漏边界场景</span> | Happy path 写得完整，"版本不存在、两版本相同、内容超长"全没列                                               | 开发写完测试才发现要补，文档反复改 |
| 翻车三：<span style="color: red; font-weight: bold;">忘了老项目约束</span>                                                   | 工程师按新需求拆，忘了"promptKey 已被 SDK 客户依赖、接口路径不能改"                                          | 改造做到一半踩了禁区，整个返工   |

### 1.2 共同根因：没有可重复的体系

为什么差距这么大？<span style="color: red; font-weight: bold;">不是技术能力问题，是大家都在凭经验做事——状态好写得全，状态差漏一半</span>。<span style="color: red; font-weight: bold;">没有可重复的体系，每次都在拍脑袋</span>。

传统软件工程师熟悉一个东西叫"接口契约"——写代码前先约定方法签名、返回结构、错误码。但**拆需求阶段没有对应的契约**，**每个人都在凭经验拍**：状态好写得全，状态差漏一半。

<span style="color: red; font-weight: bold;">本篇要给的就是拆需求阶段的"契约"——下一章的六维体系。</span>

## 2. 需求文档六维体系：把经验变成可重复的框架

需求梳理六维度：<span style="color: red; font-weight: bold;">业务目标</span>、<span style="color: red; font-weight: bold;">用户场景</span>、<span style="color: red; font-weight: bold;">接口契约</span>、<span style="color: red; font-weight: bold;">边界场景</span>、<span style="color: red; font-weight: bold;">不做什么</span>、<span style="color: red; font-weight: bold;">老项目约束</span>
<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/9dcb2cbf808f6043183ccfbf92276b21_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">

把需求文档拆成六个维度，每次拆需求都按这六个维度跑一遍。这不是发明新方法论，而是<span style="color: red; font-weight: bold;">把工程师本来就在做的事结构化</span>。**类比一下**：传统工程师写代码前有一份"检查清单"——参数校验、异常处理、日志埋点、单元测试——逐项过一遍不容易漏。六维体系就是拆需求阶段的检查清单：<span style="color: red; font-weight: bold;">跑一遍就知道哪里漏了</span>。<span style="color: red; font-weight: bold;">好处是每次产出格式一致、不漏维度、团队 review 起来快，而这恰好是 AI 最擅长的事情</span>。

> 提示：可以考虑把下面的六个维度写成 Skill 落地下来，作为团队统一的需求拆解工具。

### 2.1 六个维度对照表

以“添加Prompt版本对比”这个需求为例：

| #   | 维度                                                            | 含义                           | 是否老项目独有      | 示例（Prompt 版本对比项目需求）                                                                                           |
| --- | ------------------------------------------------------------- | ---------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------- |
| ①   | <span style="color: red; font-weight: bold;">业务目标</span>      | 一句话说清楚解决什么问题、给谁用             | 否            | 让用户快速看出 Prompt 两个版本之间的差异，定位是哪次改动引入了问题                                                                         |
| ②   | <span style="color: red; font-weight: bold;">用户场景</span>      | 典型使用场景 + 当前痛点 + 期望体验         | 否            | 工程师调试 Prompt 时切换版本看效果，目前只能各打开一次肉眼对比，期望一键对比                                                                    |
| ③   | <span style="color: red; font-weight: bold;">接口契约</span>      | 方法、路径、入参、返回、错误码              | 否            | 开发拿到这一节就能写代码                                                                                                  |
| ④   | <span style="color: red; font-weight: bold;">边界场景清单</span>    | 异常情况和它们的预期处理                 | 否            | 空值、超长、不存在、并发、跨范围……每条都要有"预期行为"                                                                                 |
| ⑤   | <span style="color: red; font-weight: bold;">老项目约束</span>     | CLAUDE.md 禁区和历史包袱里和这个需求相关的条目 | **是（老项目独有）** | 新增接口要避开禁区、新功能要兼容历史包袱                                                                                          |
| ⑥   | <span style="color: red; font-weight: bold;">不在这次范围里的事</span> | 明确这次不做什么                     | 否            | "对比两个版本"扩展开可能变成"对比 N 版本""对比附评分""导出 diff"，<span style="color: red; font-weight: bold;">不写"不做这些"AI 就会顺手做</span> |

### 2.2 六维合起来的效果

六个维度合起来就是一份完整的文档：开发拿了能直接写代码、测试拿了能直接补用例、产品拿了能直接 review。下面看看 AI 在这六维上分别能干到什么程度。

## 3. AI 能力地图：哪些活让 AI 干，哪些必须人来

<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/085f24f05f38e9876d5b07d6930c48be_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">

六维体系清楚了，下一个问题就来了：**这六个维度，AI 到底能帮到什么程度？** 答案决定了你怎么分工——AI 强的活放手让它干，AI 弱的活必须人盯。**类比一下**：传统软件团队里，后端写业务逻辑、前端做展示、DBA 管索引——每件事都有人擅长，分工才高效。AI 也一样：<span style="color: red; font-weight: bold;">它擅长"读文档 + 反推代码"这种机械活，不擅长"判断业务方向 + 产品决策"这种需要业务直觉的活</span>。看清这一点，分工才合理。

### 3.1 AI 能力三档地图

把六个维度按 AI 的得分分三档：

- 强档（85+ 分，AI 能直接交付 ）：技术契约、技术edge case、老项目约束
- 中档（60 分，AI 能起头但方向必须人定）：业务目标、用户场景
- 弱档（30 分，AI 只能列场景做不出决策）：边界场景、不做的事情

下面这张图把六维落到三档上，但这是原则并不能机械使用，还需要读下面的小节、理解它为什么强（或者弱）才能正确合理使用。
...
<!--
图片内容说明
路径：imgs/aicmigr-17-dev-01-break-down-new-requirements/88b7e272ece1f07bbde24a31b5e7e2bb_MD5.jpg
...
### 3.2 强档：AI 能直接交付的三个维度

强档的三个维度——接口契约、技术性 edge case、老项目约束——<span style="color: red; font-weight: bold;">共同点是"读得全就写得对"</span>，<span style="color: red; font-weight: bold;">而 AI 恰好每次启动都加载全部 docs/ 和 CLAUDE.md，比人记得全</span>。

| 维度                     | AI 强在哪里                                                                                                                  | 人手写的差距                              |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------- |
| <span style="color: red; font-weight: bold;">接口契约</span>               | 基于 docs/api-list.md 和现有代码，能写出和<span style="color: red; font-weight: bold;">现有接口风格一致</span>的契约；错误码沿用项目惯例、统一返回结构对、参数命名风格一致 | 人手写要查现有接口模仿，AI 30 秒搞定               |
| <span style="color: red; font-weight: bold;">边界场景的技术性 edge case</span> | 404、空值、超长、并发、不存在……这些<span style="color: red; font-weight: bold;">技术 edge case</span>AI 列得很全                              | AI 比人想得全，因为 AI 不会疲劳                 |
| <span style="color: red; font-weight: bold;">老项目约束</span>              | 基于 <span style="color: red; font-weight: bold;">CLAUDE.md （先前创建的资产）</span>和现有代码反推，不用每次都回忆                                | 人凭脑子记不住 CLAUDE.md 全文，AI 每次启动都加载全部内容 |

### 3.3 中档：AI 能起头，方向必须人来定

中档的两个维度——业务目标和用户场景——<span style="color: red; font-weight: bold;">AI 能从代码反推出一版草稿，但真实业务方向和痛点 AI 不知道，草稿只能当起点，方向必须人定</span>。

| 维度   | AI 表现                                                                        | 为什么不能直接定稿                                                      |
| ---- | ---------------------------------------------------------------------------- | -------------------------------------------------------------- |
| <span style="color: red; font-weight: bold;">业务目标</span> | 能从代码反推一个版本，但<span style="color: red; font-weight: bold;">容易写偏</span>方向       | 例如 AI 写"帮工程师调试不同版本"，但真实目标是"给运营看 Prompt 演进"，方向就错了。草稿可做起点，方向必须人定 |
| <span style="color: red; font-weight: bold;">用户场景</span> | 能根据接口推测使用场景，但<span style="color: red; font-weight: bold;">真实痛点 AI 不知道</span> | 草稿可做起点，具体痛点要人补                                                 |

### 3.4 弱档：AI 列得出场景，做不出决策

弱档的两个维度——边界场景的产品决策和"不在这次范围里的事"——AI 能把场景列出来，但怎么处理完全是产品判断，AI 不知道优先级和资源。

| 维度        | 为什么 AI 弱                                                    |
| --------- | ----------------------------------------------------------- |
| <span style="color: red; font-weight: bold;">边界场景的产品决策</span> | AI 能列"内容超长怎么办"这种场景，但这种场景怎么处理是产品决策：截断？返回 400？提示用户？<span style="color: red; font-weight: bold;">AI 不知道产品判断</span> |
| <span style="color: red; font-weight: bold;">不在这次范围里的事</span> | 完全是产品判断。AI 默认会写"做对比"<span style="color: red; font-weight: bold;">含糊一片，不知道优先级和资源</span>                          |

### 3.5 分工心法：70 + 30 = 90+

三档地图摆出来，分工心法就一句话：**AI 强的70%让 AI 承担，AI 弱的30%必须人补，整体出来 90+。**

为什么是 70 + 30？

因为 AI 强档的三个维度（接口契约、技术边界、老项目约束）本身就是需求文档里篇幅最大的部分——这些 AI 写得又快又全，压满就有 70 分。

剩下 30 分全在业务判断上，AI 永远做不了，必须人补。<span style="color: red; font-weight: bold;">跳过这 30%，文档看起来挺全，业务方一句"不是我要的"就推翻重来</span>。

人补的 30% 具体补什么？答案是三个关键判断：<span style="color: red; font-weight: bold;">业务目标方向</span>、<span style="color: red; font-weight: bold;">边界场景的产品决策</span>、<span style="color: red; font-weight: bold;">改造范围决策</span>。下一章的三步走，就是教你怎么把这 70 + 30 跑出来。

## 4. 三步走工作流：草稿 → 判断 → 定稿

讲完方法论，立刻上实战。三步走的节奏是：**AI 出草稿 → 人补判断 → AI 出定稿**。

类比一下：传统工程师写需求文档时，流程通常是"先写 PRD 草稿 → 团队评审 → 评审意见合并出最终版"。AI 接管了"写草稿"和"合并最终版"两个机械环节，让人专注最关键的一步——评审判断。下面先讲方法论，再拿一个真实需求跑一遍。

### 4.1 三步走方法论总览

<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/08655d760e57b89c6ff5250841f05bf5_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">
...
<!--
图片内容说明
路径：imgs/aicmigr-17-dev-01-break-down-new-requirements/5e0f229ac71f33bbd60794cafe89691a_MD5.jpg
用途：可视化"草稿 → 判断 → 定稿"三步走工作流及各步骤用时，帮助读者快速掌握整体节奏
内容：自上而下的流程图。Step 1（AI 出六维草稿，读 docs/ + CLAUDE.md + 现有代码，约 5 分钟）→ Step 2（人补三个关键判断：业务方向 + 产品决策 + 改造范围，约 10 分钟）→ Step 3（AI 整理定稿，格式化为正式技术文档，约 5 分钟）→ 终点"20 分钟产出一份 80+ 分需求文档"
-->
...
其他维度（接口契约、技术边界、老项目约束）AI 写的直接用。**只补这三个判断**的原因如前面能力地图所示：<span style="color: red; font-weight: bold;">AI 在数据和代码层面强，在业务判断层面弱</span>。<span style="color: red; font-weight: bold;">这三个判断都是业务判断，AI 永远做不了。</span>

### 4.2 实战背景：Prompt 版本对比需求

<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/a67f85cf8e149a12cf92738632477a71_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">

下面拿一个真实需求跑一遍三步走。需求来自 Spring AI Alibaba Admin —— 一个有版本管理但缺版本对比的系统。
...
| 约束 | 作用 |
|------|------|
| <span style="color: red; font-weight: bold;">强制读 docs/ + CLAUDE.md + 现有代码</span> | 把老项目隐性知识显性化 |
| 单独列"老项目约束"维度 | 强迫 AI 显式输出禁区和历史包袱 |
| <span style="color: red; font-weight: bold;">边界场景至少 8 条 + 每条标"预期行为"或"待产品决策"</span> | 防止 AI 偷懒只列 happy path |
...
<!--
图片内容说明
路径：imgs/aicmigr-17-dev-01-break-down-new-requirements/af2f4c2ce042cd2fc21ae3b08729688d_MD5.jpg
...
<!--
图片内容说明
路径：imgs/aicmigr-17-dev-01-break-down-new-requirements/911f81fe5aa5a973127bce2df224f2f0_MD5.jpg
...
AI 不是泛泛说要遵守约束，而是**把每一条具体到哪一节、影响什么、怎么对齐**。
这种细节，工程师手写时 90% 会忘。

##### ② 边界场景实际列了 12 条（截图只截了 5 条）

AI 不光列了基础场景（不存在、相同），还**列出了只有读了代码才知道的场景**：
...
这种**透明度比 AI 自作主张拍板要好得多**。

### 4.4 Step 2 实战：人补三个关键判断

<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/0883d4077dba442a91f948d817582d21_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">

草稿覆盖了 70%，剩下 30% 必须人补。**只 review 三个判断**：

| 判断                | 为什么必须人补              |
| ----------------- | -------------------- |
| **业务目标**的方向       | AI 在业务判断层面弱，方向必须人定   |
| **边界**场景的**产品决策** | 这些是产品决策，AI 不知道产品判断   |
| **不在这次范围**里的事     | 完全是产品判断，AI 不知道优先级和资源 |
...
| 维度           | AI 草稿默认          | 修正后                  |
| ------------ | ---------------- | -------------------- |
| 业务目标         | <span style="text-decoration: line-through;">工程师 review 自己的修改</span> | 团队多人协作下的 Prompt 演进追溯 |
| 对 diff 结果的影响 | <span style="text-decoration: line-through;">只返回内容</span>            | 返回元信息（创建时间、状态）+ 内容   |
...
这个提示词有点长，但内容是模板化的，可以在自己的项目中复用。<span style="color: red; font-weight: bold;">整个三步跑下来 20 分钟：Step 1 让 AI 跑五分钟、Step 2 做三个判断十分钟、Step 3 让 AI 整理五分钟，比手写 PRD 省两个小时</span>。

#### (2) 定稿产出预览

跑完 Step 3，会得到一份结构标准的需求文档 `docs/requirements/prompt-version-diff.md`，包含一句话总结、业务目标、用户场景、接口契约（含基本信息/入参/返回结构/错误码）、边界场景、老项目约束、不在这次范围里等七节。完整内容见后文 `8. 附录：完整需求文档定稿`。
...
| 产出物 | 路径 | 作用 |
|-------|------|------|
| 需求文档定稿 | `docs/requirements/prompt-version-diff.md`（完整内容见 `8. 附录`） | 拆改造方案的输入 |
| 三段提示词模板 | 见 4.3、4.5 节 Code Block | 可在团队内复用，下次拆需求直接套 |
| 三判断决策记录 | 见 4.4 节三张表 | 留痕，后续追溯业务决策依据 |

## 5. 最容易踩的坑：把 AI 草稿当定稿

<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/d4a83f8fc9aabeb943af92bc3d6dd40d_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">

<span style="color: red; font-weight: bold;">你可能会问：AI 草稿覆盖 70% 看起来挺全，能不能跳过 Step 2 直接用？</span>

<span style="color: red; font-weight: bold;">不能</span>。这是老项目拆需求最大的翻车点，下面拆开讲为什么。
...
<!--
图片内容说明
路径：imgs/aicmigr-17-dev-01-break-down-new-requirements/3444a390e463e7850a742043b4625280_MD5.jpg
用途：可视化"把 AI 草稿当定稿"的翻车链路，警示读者跳过 Step 2 的后果
内容：从左到右的流程图。起点是"AI 草稿覆盖 70%（集中在 AI 强的维度）"，依次经过"跳过 Step 2"→"AI 弱的维度凭默认理解填"→"业务方向偏 / 范围模糊 / 产品决策错位"，终点是"改造做完，业务方说：不是我要的"
-->

链路拆开看：草稿集中在 AI 强的维度（接口契约、技术边界、老项目约束），跳过 Step 2 后 AI 弱的维度（业务方向、产品决策、改造范围）只能凭默认理解填 —— <span style="color: red; font-weight: bold;">业务方向偏、范围模糊、产品决策错位三件套一起出现，改造做完业务方一句"不是我要的"就推翻重来</span>。
...
系列第 15 篇讲过"AI 写测试的隐性偏差"——AI 用业务直觉补测试断言，错；本篇讲 AI 用代码默认填业务目标和产品决策，错。两篇背后是同一个心法：**AI 在数据和代码层面强，在业务判断层面弱**。<span style="color: red; font-weight: bold;">和 AI 分工，不要把 AI 当定稿——这是本篇最硬的一句话</span>。

## 6. 拆需求阶段 Check List

<img src="imgs/aicmigr-17-dev-01-break-down-new-requirements/cc219f54d30b71b400c9579fbc3cb768_MD5.jpg" style="display: block; width: 800px;" alt="替换文字">
...
一个观点：到了改造的深水区，拆需求这一步千万不能偷懒。**这一步的好坏，是老项目改造效果好坏的最重要原因**——不是之一。
...
本附录是 Step 3 跑完产出的 `docs/requirements/prompt-version-diff.md` 完整内容，作为参考模板。
