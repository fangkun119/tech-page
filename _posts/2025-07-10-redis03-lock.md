...
单机 synchronized 会有分布式并发问题。以分布式扣减库存场景为例，如果不能处理好分布式并发问题，就会出现超卖问题。本文以该场景例，介绍 Redis 分布式锁。
...
### 3.4 加锁失败时等待重试： RedissonLock.tryAcquireAsync(...)
...
### 3.5 解锁线程发送信号量：`unlockInnerAsync(...)`
...
一个问题：如果解锁信号量丢失怎么办？例如占有锁的线程没有发送解锁通知、或者通知在网络上丢失了没有达到，那么等待获取锁的线程是否会永远阻塞？答案是不会，从3.4 小节的代码可以看出，订阅消息是设置阻塞时间的，不是无限等待。
...
另一方法是不使用 Redisson，而是自己手写 LUA 脚本让 Redis 执行，来等待操作同步
