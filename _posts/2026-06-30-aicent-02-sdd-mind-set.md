...
核心立场只有一句：**你不告诉它规矩，它就自己编一套**。SDD 的全部价值，就是把"自己编"变成"按规矩来"。

### 2.2 为什么需要 SDD：单次指令失效的本质

#### (1) Claude Code 没有长期记忆

很多人误以为 Claude Code"上一轮对话学过，这一轮应该记得"。**这是错觉**。它不是"忘了"，它根本没看到——每次新对话，对它来说都是从零开始。
...
写好 Prompt 是**一次性技巧**，SDD 是**贯穿项目生命周期的方法论**。两者的差距，本质上是"打一枪换一个地方"和"建一座工厂"的差距。
...
你的注意力应该放在它**真正容易犯错**的地方。一份好的 CLAUDE.md 大概两三页，每一条都是因为它在某个点上跑偏过才补上去的——没跑偏过的地方，一条都没有。
...
**核心规则**：任务规范可以覆盖模块规范（"这次不需要分页"），但**不能违反全局规范**（所有接口统一返回 `Result<T>`——任何任务都不能破坏这条）。
...
每一条单独看都不是 bug，功能完全正常。但放在一个项目里——**两个模块两种风格**。前端对接要适配两套格式；后面维护每个模块都要先搞清楚"这个模块的规矩是什么"。
...
> 单次指令的清晰度，只能解决单次输出的质量。要保证整个项目几十次、几百次 AI 输出的一致性，你需要的不是"一条好指令"，是一套规范体系——写下来、每次自动加载，让 Claude Code 不管在哪个模块、哪个阶段，都在同一套规矩下工作。
...
Hify 的 CLAUDE.md 最终大概两三页。**没有一条是"提前想出来的"，全部是在开发过程中一条一条补上去的**。没跑偏过的地方，一条都没有。
...
第1周你可能觉得"写规范好麻烦，直接让它干不就行了"。但到第三周，你会发现 Claude Code 几乎不跑偏了——**不是它变聪明了，是你的规范把所有容易踩的坑都堵上了**。后面每做一个新模块，规范帮你省下的 review 时间和返工时间，远超当初写规范的投入。
