...
部署架构从第一天就在影响你的设计决策——缓存放哪、数据库怎么连、服务之间怎么通信，全都取决于部署形态。提前想清楚，后面不返工。
...
五个子主题的共同目标：**用最低成本让系统跑起来、跑得稳、跑得久**。每个子主题都有明确的方法论结构，下面逐一展开。
...
4. **AI 的输出要校验**：AI 适合模板化任务，但复杂架构需要你的积累和判断——无脑让 AI 完全实现一个应用，当前阶段达不到。

#### (3) 注意点

- **K8s 不等于复杂**：早期可以只把 K8s 当部署壳（单副本、不引入服务网格和多副本），等触发条件到了再加复杂机制。
...
- **预判的价值在于"地图"而非"解决"**：这张表是未来出问题时的查询地图，不是一期必须落地的清单。
...
**目标**：架构不能堵死未来的扩展路，但也不能提前演进（过早优化比性能瓶颈本身的破坏力更大）。
...
2. **每个阶段只动一个层**：要么动应用层（加副本），要么动数据层（主从分离/MQ），要么动架构层（微服务拆分），不要一次全动。
...
- **架构功力不在于现在做多复杂，在于不堵未来的路**。

### 1.5 数据模型设计方法论

#### (1) 目标与思考框架

**目标**：在设计阶段确定核心表的清单与关系，具体字段留到模块开发时再定（现在定太细了后面一定会改）。
...
- **配置型表 vs 流水型表要区分**：provider/agent/workflow 是配置数据，增长慢；message/document_chunk 是流水数据，增长快，需要差异化对待。
- **向量数据单独存储**：不要把向量塞进业务数据库，单独用向量库（pgvector / Qdrant），业务库只存元数据和指向向量库的 id。
...
- **规范看起来是基础知识，但不写下来 AI 就不会遵守**：AI 不会自动给逻辑删除字段加索引、不会自动考虑分页性能、不会自动预判大表——这些必须显式写进规范。
- **数据库规范的价值在于"持续受益"**：现在花 10 分钟定好，后面每一次建表、每一次写查询都受益。
...
50 人规模意味着什么？真实压力极小，峰值并发可能就几十个请求。这个规模下，部署架构的核心诉求是"简单、可观测、留扩展口"，而不是"高可用、高并发"。
...
大部分工程师不会提前想性能瓶颈，等出了问题再救。但你作为AI的架构师，应该在设计阶段就知道瓶颈可能在哪。即使一期不处理，也要做到心里有数。
...
核心结论非常直接：**一期不需要处理任何性能瓶颈**。50 人规模的真实压力极小，所有瓶颈的触发条件都远超当前规模。

看这张表你会发现，真正在 50 人规模下能触发的只有第一行——**LLM 调用慢且占线程**。其他瓶颈的触发条件都是百万级数据或几百人并发，离我们很远。

> **关键区分**：唯一需要提前防范的是外部 LLM API 的超时和费用失控——这不是性能问题，是**稳定性和成本问题**。
...
> **本节价值**：不在于解决了什么问题，在于你脑子里有了一张系统"软肋"地图。后面出了性能问题，你不用猜，查表就行。
...
> **规范的价值**：这些规范看起来是数据库基础知识，但你不写进 CLAUDE.md，Claude Code 建表时就不会自动加 `deleted` 进索引、不会考虑分页性能、不会预判大表问题、不会给 pgvector 建 HNSW 索引。**写了，后面每一次建表、每一次写查询都自动遵守。**
