...
核心内容提炼（[`Youtube`](https://youtu.be/2gPeAQzYkYQ) `|` [`B站`](https://www.bilibili.com/video/BV1CsSUBPEfy)）：

<iframe width="560" height="315" src="https://www.youtube.com/embed/2gPeAQzYkYQ?si=E_ZT639vMApWIP92" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
下图展示了 Kafka 日志文件的完整组织结构,从 Broker 节点的 `log.dir` 目录开始,到 Topic 分区目录,再到 Segment 存储单元及其核心文件。

<img src="/imgs/kafka-04-log-index/fbb1ac38b13c5f4657bcffdbef789a0d_MD5.jpg" style="display: block; width: 100%;" alt="Kafka 日志文件组织结构">
...
下图展示了 Kafka 日志的顺序写入流程,包括历史段、写入通道和活跃段三个区域,以及滚动切换机制。

<img src="/imgs/kafka-04-log-index/f439b508e281fcbb009f83bb48212869_MD5.jpg" style="display: block; width: 100%;" alt="Kafka 日志顺序写入流程">
...
下图展示了稀疏索引的工作原理，通过索引文件映射 offset 到日志文件的物理位置。

<img src="/imgs/kafka-04-log-index/d3ca62a20eba5d92e667b3ebe8bb2833_MD5.jpg" style="display: block; width: 100%;" alt="Kafka 稀疏索引工作原理">
...
下图直观展示了两种清理策略的工作原理：Delete 策略删除整个过期 segment 文件，Compact 策略则合并相同 key 的消息并保留最新值。

<img src="/imgs/kafka-04-log-index/d4680c2f08eb9bf7e844cd643495a346_MD5.jpg" style="display: block; width: 100%;" alt="Kafka 过期文件清理策略">
...
下图展示了 Kafka 如何将消费者组的消费进度存储在内置 Topic `__consumer_offsets` 中的数据结构：

<img src="/imgs/kafka-04-log-index/513606032d07a2937ab530ce600886d2_MD5.jpg" style="display: block; width: 100%;" alt="Kafka 消费进度存储数据结构">
...
下图展示了 Linux 传统 I/O 过程中的 4 次数据拷贝和 4 次上下文切换，这正是零拷贝技术要优化的核心问题：

<img src="/imgs/kafka-04-log-index/bfdf8e899bd98d9d8dc03a2bb7348bbb_MD5.jpg" style="display: block; width: 100%;" alt="Linux 传统 I/O 数据拷贝与上下文切换">
#### (1) Linux 传统 I/O
...
下图展示了 Consumer 从 Broker 拉取数据时的 sendfile 零拷贝流程，数据完全在内核空间传输，无需经过用户态：

<img src="/imgs/kafka-04-log-index/162c2153783d123da1a202ee5a731600_MD5.jpg" style="display: block; width: 100%;" alt="sendfile 零拷贝流程">
...
下图展示了在"极致性能"与"绝对安全"之间的权衡对比，以及 Kafka 采用的务实策略：

<img src="/imgs/kafka-04-log-index/99f123a7d75c9626f89dd0873a192236_MD5.jpg" style="display: block; width: 100%;" alt="刷盘策略权衡对比">
...
理解了刷盘的基本权衡后，我们需要了解其底层实现。在 **Linux** 系统中，刷盘对应 `fsync` 系统调用：
...
Kafka 高吞吐引擎的核心设计原理如下图，涵盖了**物理基石**、**检索加速**、**空间守卫**、**状态闭环**、**I/O 抢取**五个维度。

<img src="/imgs/kafka-04-log-index/8e1c33f32b946d237deac5c94d2ad0ca_MD5.jpg" style="display: block; width: 100%;" alt="Kafka 高吞吐引擎核心设计原理">

基于这五个维度，本文系统阐述了 **Kafka 日志存储与读写机制**，并从**文件结构设计**、**索引检索机制**、**过期数据清理**、**消费进度管理**、**性能优化手段**等方面进行了完整分析。
