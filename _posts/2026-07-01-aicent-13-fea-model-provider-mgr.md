...
双段对照回顾——"想清楚"的价值（五个设计决策）+ "执行"的回报（五个基础设施复用）；标准交付流程总结；保留 3 道边界思考题（缓存一致性、DOWN 时正在进行的对话、API Key 加密）；结尾承接系列第 14 篇（设计模式重构 if-else）。
...
graph LR
    ...
    C -->|完整交付| D[第 8 章<br/>总结与边界思考<br/>3 道思考题]
...
双段对照回顾"想清楚"的价值与"执行"的回报，给出可复用的标准交付流程总结，并保留 3 道刻意未展开的边界思考题（缓存一致性、DOWN 时正在进行的对话、API Key 加密），结尾承接系列第 14 篇。
...
这三道题没有标准答案，重点不在结论，而在于用咨询模式把问题拆解清楚的过程本身。把每个问题当作一个新模块来对待：先想清楚边界、再设计数据模型、最后给出方案与取舍依据。
