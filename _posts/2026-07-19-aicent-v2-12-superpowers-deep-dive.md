...
这些不是模型不够聪明的问题。Claude、GPT、Gemini 在 benchmark 上都很强，但把它们放进真实工程项目，工程纪律还是新手水平。
...
数字和官方认可加在一起，指向同一个判断： **AI agent 缺工程纪律，是真实痛点，不是伪需求。**
...
Superpowers **是一个 agentic skills 框架** ，同时是一套 **软件开发方法论** 。它把"严肃软件工程师该做的事"打包成 Skills，强制嵌进 AI coding agent 的工作流。

说人话：它就是给 AI agent 装上一套工程纪律。不是给你一本规范手册让你自己去读，是直接装进 agent，让它必须照着做。
...
这是最关键的一个词。它不是"给你用"的工具，是"管住 agent"的约束。agent 想跳过某一步（比如不写测试直接实现），Skill 会拦住它。传统做法是靠人自觉遵守规范，Superpowers 是靠机制强制 agent 遵守。

### 2.3 它不是 SDD 工具，是 agent 的工程纪律框架

你可能会问：这跟 SDD 工具（Spec-Kit、OpenSpec）有什么区别？这个问题重要到要在这里先给一个判断。

**Superpowers 不是 SDD 工具，是 AI agent 的工程纪律框架。**
...
先用大白话建立直觉： **Skill 就是把老司机的经验写成 agent 能读懂的规则** 。传统团队里，老员工脑子里的经验靠口口相传、靠 code review 时指出问题来传递。Superpowers 把这些经验写成 SKILL.md 文件，agent 读到就能遵守。
...
这是 Superpowers 跟普通 prompt 模板的根本区别： **Skill 不靠用户手动调用，是 agent 自己识别场景后激活** 。
...
writing-skills 是 Superpowers 真正精妙的地方。它 **教 agent 怎么自己造新 Skill** 。
...
这就是 agent as contractor 哲学的落地——把 subagent 当承包商，严格验收。
...
拆完的关键发现是： **Superpowers 只改两个组件（Memory 和 Loop），其他三个全复用** 。
...
包含关系是这样的：OpenSpec（增量 SDD）+ Spec-Kit（完整 SDD）⊂ Superpowers（完整工程纪律）。
...
最核心的判断： **有用但需要筛选** 。我的个人看法是，它大概率是过渡产物的工具——不是因为它不好，而是因为 AI 编程演化太快，今天的最优解明天可能就被新机制取代。但它代表的"给 agent 装工程纪律"这个方向，不会过时。
...
最后一句话：工具会过时，工程方法论不会。Spec-Kit 可能被替代，Superpowers 也可能被替代， **但它们代表的"AI 编程需要工程纪律"这件事不会变** 。
